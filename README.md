...
- Com `enabled = true`, o aplicativo inicializa o GPIO como saida em estado desligado no start e fecha/libera o dispositivo em seguida; chamadas repetidas de `off` nao reabrem o GPIO, evitando estalos durante a troca de telas sem alerta. Durante alertas, abre o GPIO somente nos pulsos e libera logo ao voltar para `off`.
...
- Se `raw-low` e `raw-high` acionarem o buzzer, mas ele silenciar quando o processo termina/reinicia, isso indica que o seu modulo precisa do GPIO liberado/alta impedancia para ficar quieto. Com `enabled = true`, o app inicializa/desliga o GPIO no start e tambem libera o dispositivo de saida ao voltar para `off`; chamadas repetidas de `off` nao reabrem o GPIO, entao o som deve ocorrer apenas nos pulsos de alerta.
